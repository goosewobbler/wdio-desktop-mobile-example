...
| `deeplink` is trigger-only | The fixture registers no URL scheme, so `deeplink.spec.ts` only asserts `triggerDeeplink` resolves. A scheme-handling fixture is a fast-follow. |
