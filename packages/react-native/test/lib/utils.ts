...
/**
 * Resolve the WDIO output directory for the current test run.
 *
 * Layout: `packages/react-native/logs[/<scenario>]/`
 *
 * Example call sites: `currentLogDir('standard')` (standard run) or
 * `currentLogDir('contexts')` (per-test-type subdir).
 */
...
/**
 * Read every .log file in the given output directory (recursively across timestamped subdirs that
 * WDIO creates) and return the concatenated content.
 */
...
/**
 * Poll `logBaseDir` until `pattern` appears in the captured logs (with a small "stable" check so we
 * don't return mid-write). Returns true on match, false on timeout.
 */
