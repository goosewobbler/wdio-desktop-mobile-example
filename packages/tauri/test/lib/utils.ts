...
/**
 * Resolve the WDIO output directory for the current test run, derived from
 * `process.env.DRIVER_PROVIDER` (set by the wdio.<provider>.conf.ts files
 * before WDIO bootstraps).
 *
 * Layout: `packages/tauri/logs/<provider>[/<scenario>]/`
 *
 * Example call sites: `currentLogDir()` (standard scenario) or
 * `currentLogDir('multiremote')` (per-test-type subdir).
 */
export function currentLogDir(scenario?: string): string {
  const provider = process.env.DRIVER_PROVIDER;
  if (!provider) {
    throw new Error(
      'currentLogDir(): DRIVER_PROVIDER env var not set — the wdio.<provider>.conf.ts files set this; run via the test scripts.',
    );
  }
  const base = path.resolve(__dirname, '..', '..', 'logs', provider);
  return scenario ? path.join(base, scenario) : base;
}
 ...
 *
 * Trimmed-down version of the upstream helper at
 * `~/Workspace/wdio-desktop-mobile/e2e/lib/utils.ts` — same semantics, but
 * without the spawn/retry helpers we don't use here.
 */
